...
/*
// Convierte caracteres UTF-8 (acentos, ñ) a ISO-8859-1 para gofpdf
...
// GenerateInvoicePDF genera el PDF de una factura con sus detalles
func GenerateInvoicePDF(facturaData *dto.GetFacturaByIdRow, detallesData []dto.DetalleFacturaByFacturaRows) (*gofpdf.Fpdf, error) {
	...
	pdf.SetTitle("Factura Río Perdido Tours", false)
	pdf.AddPage()
	...
	pdf.Cell(50, 10, utf8ToISO88591("Río Perdido Tours"))
	pdf.SetFont("Arial", "", 10)
	pdf.SetXY(10, 22)
	pdf.MultiCell(80, 5, utf8ToISO88591("CRM Reservas, Tours y Facturación\nPailas, Guanacaste, Costa Rica\nTel: +506 8888-8888\nEmail: info@rioperdido.com"), "", "L", false)
	...
	pdf.SetXY(130, 25)
	pdf.Cell(0, 6, fmt.Sprintf("Factura N°: %06d", facturaData.Idfactura))
	pdf.SetXY(130, 31)
	pdf.Cell(0, 6, "Fecha: "+facturaData.Fechafactura.Format("02/01/2006"))
	...
	pdf.Cell(40, 8, "Facturar a:")
	pdf.SetFont("Arial", "", 11)
	pdf.SetXY(10, 54)
	nombreCliente := fmt.Sprintf("%s %s %s", facturaData.Nombrepersona, facturaData.Apellido1, facturaData.Apellido2)
	pdf.Cell(0, 6, utf8ToISO88591(nombreCliente))

	// --- Tabla de detalles ---
	pdf.SetY(70)
	pdf.SetFont("Arial", "B", 10)
	pdf.SetFillColor(220, 220, 220)
	pdf.CellFormat(70, 7, utf8ToISO88591("Descripción"), "1", 0, "L", true, 0, "")
	pdf.CellFormat(20, 7, "Cant.", "1", 0, "C", true, 0, "")
	pdf.CellFormat(30, 7, "Precio Unit.", "1", 0, "R", true, 0, "")
	pdf.CellFormat(30, 7, "Descuento", "1", 0, "R", true, 0, "")
	pdf.CellFormat(40, 7, "Subtotal", "1", 1, "R", true, 0, "")

	pdf.SetFont("Arial", "", 10)

	var subtotal float64
	for _, item := range detallesData {
		lineaSubtotal := (float64(item.CantTour) * item.PrecioTour) - item.Descuento
		subtotal += lineaSubtotal

		pdf.CellFormat(70, 7, utf8ToISO88591(item.NombreTour), "LR", 0, "L", false, 0, "")
		pdf.CellFormat(20, 7, strconv.Itoa(int(item.CantTour)), "LR", 0, "C", false, 0, "")
		pdf.CellFormat(30, 7, fmt.Sprintf("CRC %.2f", item.PrecioTour), "LR", 0, "R", false, 0, "")
		pdf.CellFormat(30, 7, fmt.Sprintf("CRC %.2f", item.Descuento), "LR", 0, "R", false, 0, "")
		pdf.CellFormat(40, 7, fmt.Sprintf("CRC %.2f", lineaSubtotal), "LR", 1, "R", false, 0, "")
	}
	...
	// --- Cálculo de totales ---
	pdf.SetFont("Arial", "", 11)
	ivaAmount := subtotal * facturaData.Iva / 100
	...
	pdf.CellFormat(35, 7, fmt.Sprintf("CRC %.2f", subtotal), "", 1, "R", false, 0, "")

	pdf.SetXY(rightX, y+7)
	...
	pdf.Cell(35, 7, "TOTAL:")
	pdf.CellFormat(35, 7, fmt.Sprintf("CRC %.2f", total), "", 1, "R", false, 0, "")

	// --- Pie de página ---
	pdf.SetY(-25)
	pdf.SetFont("Arial", "I", 9)
	pdf.MultiCell(0, 5,
		utf8ToISO88591("Esta factura es un comprobante de pago y está sujeta a los términos y condiciones de nuestros servicios.\n¡Gracias por su visita!"),
		"", "C", false)
